...
class BarqCurve(OptimizableSpaceCurve):
    ...
    def __init__(self, *, adj_target, n_free_points, pgf_mod=None, prs_fun=None):
        """
        ...
        super().__init__(
            curve=barq_curve, order=0, interval=[0, 1], deriv_array_fun=barq_derivs_fun
        )
    ...
    def _validate_curve_point(self, point, t_val):
        ...
        if numpy.any(numpy.isnan(point_numpy)) or numpy.any(numpy.isinf(point_numpy)):
            raise ValueError(
                f"Target curve returned invalid values (NaN/Inf) at t={t_val}. "
                f"Point: {point}. Check your target_curve function."
            )
        return jnp.array(point)

    def _generate_target_points_for_optimization(
        self, target_curve, target_points, n_opt_points
    ):
        """
    ...
    def _fit_curve_to_control_points(
        self,
        target_curve=None,
        target_points=None,
        n_samples=100,
    ...
    def initialize_parameters(
        self,
        *,
        init_free_points=None,
        init_pgf_params=None,
        init_prs_params=None,
        seed=None,
        ...
        curve_fitting_requested = (
            fit_target_curve is not None or fit_target_points is not None
        )
        ...
        if init_free_points is None:
            ...
            init_free_points = init_free_points / numpy.linalg.norm(
                init_free_points, axis=0
            )
            ...
            if init_free_points.shape[0] != self.n_free_points:
                raise ValueError(
                    "Inconsistent number of free points with"
    ...
    def save_bezier_control_points(self, filename):
        ...
        points = jnp.vstack([[barq_angle, -1.0, -1.0], points])
