...
K_RANGE = jnp.arange(
    -settings.options["ANGLE_K_MAX"], settings.options["ANGLE_K_MAX"] + 1
)
...
def calculate_frenet_dict(deriv_array):
    ...
    vec_idx = (
        jnp.flatnonzero(
            ...
            size=settings.options["NUM_DERIVS"] - 1,
        )[0]
        + 1
    )
...
def calculate_singularity_indices(frenet_dict):
    ...
    singularity_idxs = (
        jnp.flatnonzero(
            B_prod[1:] < -0.9, size=settings.options["NUM_SINGLS"], fill_value=-1
        )
        + 1
    )
...
def calculate_curve_area(frenet_dict):
    ...
    curve_area_integrand = jnp.cross(frenet_dict["curve"], r_prime_num, axis=1)
...
def calculate_total_length(frenet_dict):
    total_length = jnp.trapezoid(frenet_dict["speed"], frenet_dict["x_values"])
...
def calculate_total_curvature(frenet_dict):
    total_curvature = jnp.trapezoid(
        frenet_dict["speed"] * frenet_dict["curvature"], frenet_dict["x_values"]
    )
...
def calculate_total_torsion(frenet_dict):
    total_torsion = jnp.trapezoid(
        frenet_dict["speed"] * frenet_dict["torsion"], frenet_dict["x_values"]
    )
...
def calculate_cfi_value(frenet_dict):
    ...
    curve_energy = jnp.trapezoid(
        frenet_dict["speed"] * curve_energy_integrand, frenet_dict["x_values"]
    )
...
def vector_int_cumul(vector, x_values):
    ...
    return scipy.integrate.cumulative_simpson(y=vector, x=x_values, axis=0, initial=0)
...
def calculate_cumulative_length(frenet_dict):
    ...
    return scipy.integrate.cumulative_simpson(
        y=frenet_dict["speed"], x=frenet_dict["x_values"], initial=0
    )
