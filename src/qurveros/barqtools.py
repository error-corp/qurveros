...
def make_barq_fun(adj_target, pgf_mod, prs_fun):
    ...
    def barq_fun(params):
        ...
        gate_fixing_points = point_gate_fixing(
            params["free_points"][:2, :], pgf_params, adj_target
        )

        input_points = jnp.vstack([gate_fixing_points, params["free_points"][2:, :]])
...
def point_gate_fixing(initial_points, pgf_params, adj_target):
    ...
    for fix_param in [
        ...
        pgf_params[fix_param] = jnp.clip(
            pgf_params[fix_param], settings.options["FIX_NORM"]
        )

    left_tangent = initial_points[0, :] / frametools.vector_norm(initial_points[0, :])

    left_binormal = initial_points[1, :] / frametools.vector_norm(initial_points[1, :])

    left_binormal = (
        pgf_params["left_binormal_fix"] * left_binormal
        + pgf_params["left_binormal_aux"] * left_tangent
    )

    total_rotation = adj_target @ beziertools.bezier_adj(
        initial_points[0, :], initial_points[1, :], angle=0
    )

    right_tangent = -total_rotation[-1, :]

    right_binormal = (
        jnp.sin(pgf_params["barq_angle"]) * total_rotation[0, :]
        + -1 * jnp.cos(pgf_params["barq_angle"]) * total_rotation[1, :]
    )

    right_binormal = (
        pgf_params["right_binormal_fix"] * right_binormal
    ...
    for vector in frame_vectors:
        for location in vector_locations:
            for param_type in vector_param_types:
                pgf_params[location + "_" + vector + "_" + param_type] = 1.0

    pgf_params["barq_angle"] = jnp.pi
    pgf_params["right_binormal_aux"] = 0.0
    pgf_params["left_binormal_aux"] = 0.0
